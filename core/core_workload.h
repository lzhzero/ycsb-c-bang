...
public:
	...
	virtual std::vector<ycsbc::DB_BASE::KVPairInt> NextTransactionKVsInt();
...
inline std::vector<ycsbc::DB_BASE::KVPair> CoreWorkload::NextTransactionKVs() {
	...
	for (size_t i = 1; i <= num; ++i) {
		result.push_back(
				ycsbc::DB_BASE::KVPair(NextTransactionKey(),
...
inline std::vector<ycsbc::DB_BASE::KVPairInt> CoreWorkload::NextTransactionKVsInt() {
	std::vector<ycsbc::DB_BASE::KVPairInt> result;
	size_t num = keynum_chooser_->Next();
	for (size_t i = 1; i <= num; ++i) {
		result.push_back(
				ycsbc::DB_BASE::KVPairInt(NextTransactionKeyInt(),
						std::string(field_len_generator_->Next(),
								utils::RandomPrintChar())));
	}
	return result;
