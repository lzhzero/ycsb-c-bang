...
void CoreWorkload::Init(const Properties &p) {
  ...
  read_all_fields_ = Util::StrToBool(p.GetProperty(READ_ALL_FIELDS_PROPERTY,
                                                    READ_ALL_FIELDS_DEFAULT));
  write_all_fields_ = Util::StrToBool(p.GetProperty(WRITE_ALL_FIELDS_PROPERTY,
  ...
  if (scan_len_dist == "uniform") {
    ...
    throw Util::Exception("Distribution not allowed for scan length: " +
        scan_len_dist);
  }
...
Core::Generator<uint64_t> *CoreWorkload::GetFieldLenGenerator(
    const Properties &p) {
  ...
  if(field_len_dist == "constant") {
    ...
    throw Util::Exception("Unknown field length distribution: " +
        field_len_dist);
  }
}
